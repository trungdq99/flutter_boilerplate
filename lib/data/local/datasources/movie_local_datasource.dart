...
class MovieLocalDataSource extends IMovieDataSource {
  final _moviesStore = intMapStoreFactory.store(LocalConsts.movieStoreName);
  ...
  Future<MovieModel?> getByImdbID(String? imdbID) async {
    try {
      ...
      return MovieModel.fromJson(recordSnapshot.value);
    } catch (err) {
  ...
  Future<List<MovieModel>?> getList() async {
    try {
      ...
      return recordSnapshots.map((e) => MovieModel.fromJson(e.value)).toList();
    } catch (err) {
      throw LocalServerException(err);
    }
  }
  ...
  Future<void> save(MovieModel movieModel) async {
    try {
      if (await getByImdbID(movieModel.imdbID) == null) {
        await _moviesStore.add(database, movieModel.toJson());
      }
    } catch (err) {
      throw LocalServerException(err);
    }
